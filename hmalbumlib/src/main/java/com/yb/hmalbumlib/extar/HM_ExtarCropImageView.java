...
public class HM_ExtarCropImageView extends ImageView {
    private static final String TAG = HM_ExtarCropImageView.class.getSimpleName();
    ...
    public HM_ExtarCropImageView(Context context) {
        this(context, null);
    }

    public HM_ExtarCropImageView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public HM_ExtarCropImageView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void drawOverlay(Canvas canvas) {
        ...
        if (!mIsAnimating
                && (mCropMode == CropMode.CIRCLE || mCropMode == CropMode.CIRCLE_SQUARE || mCropMode== CropMode.SMOOTHROUND || mCropMode== CropMode.SMOOTHROUND_SQUARE)) {
            path.addRect(
                    overlayRect,
                    Path.Direction.CW
            );
            if (mCropMode== CropMode.SMOOTHROUND || mCropMode== CropMode.SMOOTHROUND_SQUARE){
                path.addPath(getSmoothRoundPath(mFrameRect));
    ...
    public Bitmap getCroppedBitmap() {
        ...
        if (mCropMode== CropMode.SMOOTHROUND){
            Bitmap circle = getSmoothRoundBitmap(cropped);
            if (cropped != getBitmap()) {
                cropped.recycle();
            }
            cropped = circle;
    ...
    public Bitmap getSmoothRoundBitmap(Bitmap square) {
        if (square == null) return null;
        Bitmap output = Bitmap.createBitmap(square.getWidth(), square.getHeight(),
                Bitmap.Config.ARGB_8888);
        ...
        canvas.drawPath(getSmoothRoundPath(new RectF(rect.left,rect.top,rect.right,rect.bottom)), paint);
        paint.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.SRC_IN));
        canvas.drawBitmap(square, rect, rect, paint);
        return output;
    }
    public Path getSmoothRoundPath(RectF rectF){
        float minWidth = Math.min(rectF.width(),rectF.height());
        float center = (minWidth / 2.0f);
        float controlPrint1 = (center / 3.0f);
        float controlPrint2 = minWidth - controlPrint1;
        // Local Colors
        Path bezierPath = new Path();

        bezierPath.moveTo(center+rectF.left, rectF.top);
        bezierPath.cubicTo(controlPrint1+rectF.left, rectF.top, rectF.left, controlPrint1+rectF.top , rectF.left, center+rectF.top );

        bezierPath.lineTo(rectF.left, rectF.height()- center+rectF.top );
        bezierPath.cubicTo(rectF.left, controlPrint2+rectF.top , controlPrint1+rectF.left, minWidth+rectF.top , center+rectF.left, minWidth+rectF.top );

        bezierPath.lineTo(rectF.width()- center+rectF.left, rectF.height()+rectF.top );
        bezierPath.cubicTo(controlPrint2+rectF.left, minWidth+rectF.top , minWidth+rectF.left, controlPrint2+rectF.top , minWidth+rectF.left, center+rectF.top );

        bezierPath.lineTo(rectF.width()+rectF.left,center+rectF.top);
        bezierPath.cubicTo(minWidth+rectF.left, controlPrint1+rectF.top, controlPrint2 +rectF.left, rectF.top, center+rectF.left , rectF.top);

        bezierPath.lineTo(minWidth+rectF.left,rectF.top);
            ...
            public void run() {
                ...
                // Use file for crop
                    ...
                    if (mCropMode == CropMode.CIRCLE) {
                        ...
                        cropped = circle;
                    }else if (mCropMode== CropMode.SMOOTHROUND){
                        Bitmap circle = getSmoothRoundBitmap(cropped);
                        if (cropped != getBitmap()) {
                            cropped.recycle();
                        }
    ...
    public enum CropMode {
        FIT_IMAGE(0), RATIO_4_3(1), RATIO_3_4(2), SQUARE(3), RATIO_16_9(4), RATIO_9_16(5), FREE(
                6), CUSTOM(7), CIRCLE(8), CIRCLE_SQUARE(9),SMOOTHROUND(10), SMOOTHROUND_SQUARE(11);
