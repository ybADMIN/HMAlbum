 ...
 * @since 1.0.0
 */
public final class HM_StorageUtil {

	private static final String EXTERNAL_STORAGE_PERMISSION = "android.permission.WRITE_EXTERNAL_STORAGE";
	private static final String INDIVIDUAL_DIR_NAME = "hmalbum";
	private static final String TAG = "HM_StorageUtil";
	private HM_StorageUtil() {
	}
	...
	private static File getExternalCacheDir(Context context) {
		...
		if (!appCacheDir.exists()) {
			if (!appCacheDir.mkdirs()) {
				Log.w(TAG,"Unable to create external cache directory "+appCacheDir.getAbsoluteFile().toString());
				return null;
			}
			try {
				new File(appCacheDir, ".nomedia").createNewFile();
			} catch (IOException e) {
				Log.i(TAG,"Can't create \".nomedia\" file in application external cache directory "+e.getMessage());
			}
		}
		return appCacheDir;
	}
	 ...
	 * 相机拍照图片保存路径
	 * @param context
	 * @return /Android/data/"package"/cache/Updata
	 */
	public static File getAlbumDir(Context context){
		return getOwnCacheDirectory(context, "/hm_album",true);
	}
